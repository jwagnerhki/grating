    ...
    for (i = 0; i < iLen; ++i)
    {
        ...
        for (j = 0; j < NUM_FREQS; ++j)
        {
            ...
            cDataReX += SCALE_FACTOR
                        * (0.001 * cos(2 * M_PI * afFreqX[j] * i / F_S));
            cDataImX += SCALE_FACTOR
                        * (0.001 * sin(2 * M_PI * afFreqX[j] * i / F_S));
        }
        ...
        for (j = 0; j < NUM_FREQS; ++j)
        {
            ...
            cDataReY += SCALE_FACTOR
                        * (0.002 * cos(2 * M_PI * afFreqY[j] * i / F_S));
            cDataImY += SCALE_FACTOR
                        * (0.002 * sin(2 * M_PI * afFreqY[j] * i / F_S));
        }
...
void PrintUsage(const char *pcProgName)
{
    (void) printf("Generates a data file with an 8-bit-quantized "
                  "dual-polarization complex signal \n"
                  "containing %d tones and uniform noise sampled at %g MHz."
                  "\n\n", 
                  NUM_FREQS, F_S);
    ...
    (void) printf("Number of complex-valued samples in each \n");
    (void) printf("                                         ");
    (void) printf("polarization.\n");
